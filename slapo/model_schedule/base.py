...
def shard_attention(
    sch, name=["q_proj", "k_proj", "v_proj", "out_proj"], attr="num_heads"
):
    if len(name) == 4:
        q, k, v, out = name
    ...
    elif len(name) == 2:  # q,k,v have been fused
        qkv, out = name
    ...
    sch = sch[path]
    if hasattr(sch.mod, attr):
        setattr(sch.mod, attr, getattr(sch.mod, attr) // sch.world_size)
    else:
        raise ValueError(f"Invalid attribute {attr}")


def shard_mlp(sch, name=["c_fc", "c_proj"]):
    l1, l2 = name
