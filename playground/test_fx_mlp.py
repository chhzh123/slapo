...
sol = Solver(fx.symbolic_trace(MLP()), p=8)
sol.solve([torch.randn(seq, d)])
sys.exit()
...
start = time.time()
for _ in range(10):
    mod_reshard(torch.randn(8, seq, d).to(device=device))
end = time.time()
if dist.get_rank() == 0:
...
start = time.time()
for _ in range(10):
    mod_mega(torch.randn(8, seq, d).to(device=device))
end = time.time()
if dist.get_rank() == 0:
