...
class Solver():
    ...
    def construct_z3_problem(self):
        ...
        for op in self.z3_graph:
            ...
            # input constraints
            constraints = [z3.And(lhs == ShardSpec("RR").id, rhs == ShardSpec("RS").id),
        ...
        for i, op in enumerate(self.z3_graph):
            prev = outputs[i]
            curr = bitvecs[f"{self.z3_graph[i + 1].name}_lhs"] if i < len(self.z3_graph) - 1 else ShardSpec("RR").id
            reshard_costs.append(calculate_reshard_cost_z3(prev, curr, op.out_size))

        self.cost = sum(comm_costs) + sum(reshard_costs)

        self.goal += [bitvecs[f"{self.z3_graph[0].name}_lhs"] == ShardSpec("RR").id] # input should not be sharded
    ...
    def solve(self, inputs):
        ...
        for it in range(3):
            ...
            for i, op in enumerate(self.z3_graph):
                ...
                print(f"{name}: {ShardSpec(lhs)} x {ShardSpec(rhs)} = {ShardSpec(output)}")
                comm_cost = op.calculate_comm_cost(lhs, rhs)
                next_inp = results[f"{self.z3_graph[i + 1].name}_lhs"] if i < len(self.z3_graph) - 1 else ShardSpec("RR").id
