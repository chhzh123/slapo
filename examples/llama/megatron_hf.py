...
def model_provider(pre_process=True, post_process=True):
    ...
    if "llama" not in model_name:
        raise RuntimeError(f"Only llama is supported for now, got {model_name}")
    ...
    class LLaMAWithLMHead(torch.nn.Module):
        def __init__(self):
            super().__init__()
...
def get_batch(data_iterator):
    ...
    # is broadcast. The required shape for HF LLaMA is (batch, 1, 1, seq_length).
    # (1, 1, S, S) -> (1, 1, S, 1)
